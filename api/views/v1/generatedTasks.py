...
@method_decorator(apiKeyRequired, name="dispatch")
class GeneratedQuestionsAPIView(APIView):
    def post(self, request):
        responseData, httpStatus = self.generateQuestions(request.data)
        return Response(responseData, status=httpStatus)
    

    @staticmethod
    def generateQuestions(projectData):
         # Check
        serializer = GeneratedTaskSerializer(data=projectData)
        if not serializer.is_valid():
            return serializer.errors, status.HTTP_400_BAD_REQUEST
            ...
            messages=[
                {
                    "role": "system",
                    "content": """You are an assistant for quayside.app, a project management team. 
                    You are given, as input, information regarding a project that a single 
                    person or a team wants to take on. 
                    Eventually you are going to break the project down into tasks and subtasks.
                    Ask the two questions which, when answered, will give you the clarification 
                    that will help you with the task of breaking down the project.
                    The questions should be very hands off simple questions.
                    You should NOT ask a question that needs more than 3 words to answer.
                    You should NOT ask to have the project broken down for you.
                    Put each question on its own line. 
                    Respond with the questions and nothing but the questions. 
                    Don't number the questions.
                    Keep the questions to 8 or less words.
                    """,
                },
                {"role": "user", "content": f"Project Name: {projectName}\nProject Description: {projectDescription}"},
            ],
